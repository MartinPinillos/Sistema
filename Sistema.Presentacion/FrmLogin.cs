    ...
    public partial class FrmLogin : Form
    {
        ...
        private void BtnAcceder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(Tabla.Rows.Count<=0) //Si mi objeto Tabla no tiene al menos 1 registro valido con mail y clave voy a mostrar el
                                        //siguiente mensaje
                {
                    MessageBox.Show("El email o la clave es incorrecta.", "Acceso al Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else  //Si tenemos al menos 1 registro
                {
                    if(Convert.ToBoolean(Tabla.Rows[0][4]) == false) //El 4 es la columna "Estado"
                    {
                    ...
                    else
                    {
                        ...
                        Variables.IdUsuario = Convert.ToInt32(Tabla.Rows[0][0]); //vamos a enviar ese usuario autenticado
                        ...
                        Frm.Show();   //Para que se muestre el FRMPrincipal
                        this.Hide(); // para ocultar el formulario Acceso
